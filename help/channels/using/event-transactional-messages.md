...
uuid: 7906c30f-e8d3-4edd-adbe-e76cb109bcaa
...
discoiquuid: 916eeb09-6c19-4be9-9f4b-17d4fa2ea43e
isreadyforlocalization: false
navTitle: Event transactional messages
publishexternaldate: 2018-11-20
sha1: ab42fa2110ef5effdef64f956a2200ee189876d1
index: y
internal: n
snippet: y
---
...
## Using loops in a transactional message {#using-loops-in-a-transactional-message}

You can create loops referencing one or more data collections in the content of a transactional email. For example, in a cart abandonment email you can include a list of all products that were in the users' carts when they left your website, with an image, the price, and a link to each product.

>[!NOTE]
>
>Loops are only available when editing transactional email messages through the [Email Designer](../../designing/using/about-email-content-design.md#using-the-email-designer) interface. Adobe Campaign does not support nested loops, meaning that you cannot include a loop inside another loop.

To add a list of abandoned products in a transactional message, follow the steps below.

### Defining a loop {#defining-a-loop}

Before being able to use a loop in a transactional message, you need to define at the event level the list of products and the fields for each product of the list you want to display. For more on this, see [Defining data collections](../../administration/using/configuring-transactional-messaging.md#defining-data-collections).
...
1. Select the structure component you just created and click the **Create a loop** icon from the contextual toolbar.

   ![](assets/message-center_loop_create.png)

   The structure component is highlighted with an orange frame and the **Loop** settings are displayed in the left palette.
...
1. Select how the elements of the collection will be displayed:
   ...
   >[!NOTE]
   >
   >The **Column** option is only available when using a multicolumn structure component (**2:2 column**, **3:3 column** and **4:4 column**). When editing the loop, only fill in the first column: the other columns will not be taken into account. For more on selecting structure components, see [Editing the email structure](../../designing/using/defining-the-email-structure.md#editing-the-email-structure).

1. Select the data collection you created when configuring the event related to the transactional message. You can find it under the **Context** > **Real-time event** > **Event context** node.
...
1. Use the **First item of loop** drop-down list to select which element will start the list displayed in the email.

   For example, if you select 2, the first item of the collection will not be displayed in the email. The loop will start on the second item.

1. Select the maximum number of items to display in the list.

   >[!NOTE]
   >
   >If you want the elements of your list to be displayed vertically (**Column**), the maximum number of items is limited according to the selected structure component (2, 3 or 4 columns). For more on selecting structure components, see [Editing the email structure](../../designing/using/defining-the-email-structure.md#editing-the-email-structure).

### Populating the loop {#populating-the-loop}

To display a list of products coming from the event link to the transactional email, follow the steps below.

For more on creating a collection and related fields when configuring the event linked to the transactional email, see [Defining data collections](../../administration/using/configuring-transactional-messaging.md#defining-data-collections).

1. Select the image component you inserted, select **Enable personalization** and click the pencil in the Settings pane.

   ![](assets/message-center_loop_image.png)

1. Select **Add personalization field** in the **Image source URL** window that opens.
...
1. At the desired position, select **Insert personalization field** from the contextual toolbar.

   ![](assets/message-center_loop_product.png)

1. From the **Context** > **Real-time event** > **Event context** node, open the node corresponding to the collection that you created (here **Product list**) and select the field that you created (here **Product name**). Click **Confirm**.
...
1. Select some text and select **Insert link** from the contextual toolbar.

   ![](assets/message-center_loop_link_insert.png)

1. Select **Add personalization field** in the **Insert link** window that opens.
...
1. Select the structure component on which the loop is applied and select **Show fallback** to define a default content.
...
1. From the Settings pane, edit the styles for the loop. For more on this, see [Editing email styles](../../designing/using/editing-email-styles.md).
1. Preview the email using a test profile linked to the relevant transactional event and for which you defined collection data. For example, add the following information in the **Event data** section for the test profile you want to use:
...
Now, as soon as the "Cart abandonment" event is triggered, it automatically prompts a message containing the recipient's title and last name, the cart URL, the last product consulted or a list of products if you defined a loop, and the total cart amount to be sent.
