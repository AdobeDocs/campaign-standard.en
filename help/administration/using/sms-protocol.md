...
This document will walk you through the connection set up a between Adobe Campaign and a SMPP provider.
...
Adobe Campaign Standard supports the SMPP protocol version 3.4. This is a widespread protocol that allows sending SMS to a provider (SMSC) and receiving SMS as well as receipts. For more on this, refer to the [SMPP documentation](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).
...
* **Transceiver (abor TRX)**: a single TCP connection is used for transmitting and receiving messages.

>[!NOTE]
>
>TRX is preferred for Adobe Campaign Standard as it reduces the number of connections and simplifies connection recovery in case of failure.
...
* **short_message**: text of the message. For SR, this contains data described in the Appendix B of the SMPP protocol specification. See [SR error management](../../administration/using/sms-protocol.md#sr-error-management) for more details.
...
* Make sure that you know which characters belong to which encoding. GSM7 does not fully support diacritical marks (accents). Especially in French, where é and è are part of GSM7, but ê, â or ï are not. The same applies to Spanish.
...
See the [SMS text encoding](../../administration/using/sms-protocol.md#sms-text-encoding) section for details on text encoding.
...
#### Matching MT, SR and broadlog entries {#matching-mt-sr}
...
The ID given by the provider is stored in the `sProviderId` column of `nmsBroadLogExec` table. SR always arrive after the MT was successfully sent and acknowledged, but can sometimes arrive out of order, known in Adobe Campaign as outstanding SR. The processing thread stores these SR temporarily in RAM until the complete information arrives.
...
* The broadlog message mechanism is used to qualify the error and find the broadlog message ID.
* The broadlog is updated with all the information above.
